...
export function MultiViewDialog({
	...
	const { isWalletOpen, setIsWalletOpen } = use(SelectedAccountContext);
	...
	if (!isMobile) {
		return (
			<Sheet open={isWalletOpen} onOpenChange={handleOpenChange}>
				...
				<SheetContent className="border-border my-auto mr-2 h-[calc(100%-24px)] rounded-2xl border p-2 outline-none font-outfit">
					<MultiView
						height={height}
						direction={direction}
						ref={containerRef}
	...
	return (
		<Drawer open={isWalletOpen} onOpenChange={handleOpenChange}>
			...
			<DrawerContent className="bg-background border-border max-h-[90vh] min-h-[20vh] w-full p-2 font-outfit">
				<MultiView
					height={height}
					direction={direction}
					ref={containerRef}
